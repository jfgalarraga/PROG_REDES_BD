...
function checkMatch(start, end) {
    ...
    if (start.dataset.pair === end.dataset.pair) {
        ...
        // Hacer desaparecer los elementos después de un breve retraso
        setTimeout(() => {
            start.style.visibility = "hidden";
            end.style.visibility = "hidden";

            // Verificar si todos los elementos están emparejados y ocultos
            const allMatched = Array.from(document.querySelectorAll(".item")).every(item => 
                item.classList.contains("matched") && item.style.visibility === "hidden"
            );

            if (allMatched) {
                messageContainer.textContent = "¡Muy buen trabajo!";
                messageContainer.className = "message success";

                // Mostrar el botón para volver a intentar
